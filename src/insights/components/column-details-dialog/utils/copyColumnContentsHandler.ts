...
import { ApiRefs } from "src/insights/redux/types/ui/dataExplorer";

export const copyColumnContentsHandler = (column: string, apiRefs: RefObject<ApiRefs>) => {
  const result = apiRefs.current.reduce((acc, apiRef) => {
    ...
    if (api.state.filter.filterModel.items.length > 0) {
      ...
      const rows = Object.keys(filteredRowsLookup).reduce((acc, filteredRowId) => {
        return filteredRowsLookup[filteredRowId] ? [...acc, api.getRow(filteredRowId)] : acc;
      }, [] as string[]);
      const rowsValues = rows.length > 0 ? rows.map((row) => row[column] ?? "-") : null;
