...
const getColumnDetailsDefinitionsLabels: { [key: string]: string } =
  appLabels.getColumnDetailsDefinitions;
const getIndexDetailsItemsLabels: { [key: string]: string } =
  appLabels.getIndexDetailsItems;
...
export const copyTableContentsHandler = (apiRefs: RefObject<ApiRefs>) => {
  ...
  const result = apiRefs.current.reduce<string[]>((acc, apiRef) => {
    ...
    const columnLabels =
      acc.length === 0 ? [...indexLabels, ...dataColumnLabels].join(',') : null;

    const indexValues = indexIds.map((column) =>
      column in indexDetails
        ? indexDetails[column as keyof typeof indexDetails].toString()
        : '-',
    );
    const rowsValues =
      rows.length > 0
