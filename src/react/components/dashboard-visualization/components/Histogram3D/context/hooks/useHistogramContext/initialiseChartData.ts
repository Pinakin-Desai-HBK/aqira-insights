...
import { Dimensions } from "src/react/redux/types/redux/workspaces";
import { GetSchema, VIS_ERROR_TYPES } from "src/react/components/dashboard-visualization/components/Shared/types";
...
export const initialiseChartData = async ({
  ...
  dashboardId: string;
  visualizationId: string;
  getSchema: GetSchema<"Histogram3D">;
  ...
  const fetchData = async () => {
    try {
      const fetchedData = await fetchChartData({
        ...
        queryChannelNames: [],
        properties: { connection: "absolute", type: "Histogram3D" },
        dashboardId,
        schema,
        visualizationId
      });
  ...
  const chartDataWithoutRefs = prepareChartData({
    histogramChartData: fetchedData.data,
    dataSetId,
